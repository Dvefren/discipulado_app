...
export function Sidebar({ user }: SidebarProps) {
  ...
  return (
    <aside className="w-[210px] min-w-[210px] bg-gray-50 border-r border-gray-200 flex flex-col h-full">
      {/* App Logo */}
      ...
      <div className="px-3 py-3 border-t border-gray-200">
        <div className="flex items-center gap-2 px-3 py-2">
          <div className="w-7 h-7 rounded-full bg-purple-600 flex items-center justify-center text-white text-[11px] font-medium">
            {user.name?.charAt(0).toUpperCase() || "U"}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-medium text-gray-900 truncate">{user.name}</p>
            <p className="text-[11px] text-gray-400 truncate">{(user as any).role}</p>
          </div>
          <button
