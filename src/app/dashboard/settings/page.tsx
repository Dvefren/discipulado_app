...
export default function SettingsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Delete this question? Student answers using this question will remain in their profiles but won't be displayed.")) return;
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground/70 mt-0.5">
            Manage church profile questions for students
          </p>
        </div>
      </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 text-sm border border-border rounded-lg outline-none focus:border-ring bg-card"
                  >
        ...
        {questions.length === 0 ? (
          ...
          <div className="divide-y divide-border/30">
            {questions.map((q, i) => (
              ...
              >
                {/* Order number */}
                <span className="text-xs text-muted-foreground/40 font-medium w-5 text-center">
                  {i + 1}
                </span>
                ...
                {editingId === q.id ? (
                  <div className="flex-1 space-y-2">
                    <input
                      ...
                      className="w-full px-3 py-1.5 text-sm border border-border rounded-lg outline-none focus:border-ring"
                    />
                    <div className="flex gap-2 items-center">
                      <select
                        value={editType}
                        onChange={(e) => setEditType(e.target.value)}
                        className="px-2 py-1 text-xs border border-border rounded-lg outline-none bg-card"
                      >
                        <option value="text">Free text</option>
                      ...
                      {editType === "select" && (
                        <input
                          ...
                          placeholder="Sí, No, En proceso"
                          className="flex-1 px-2 py-1 text-xs border border-border rounded-lg outline-none focus:border-ring placeholder:text-muted-foreground/40"
                        />
                      )}
                      <button
                        onClick={handleEdit}
